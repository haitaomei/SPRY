...
public class StreamScheduling {
     ...
     * included
     */
    public static boolean scheduleStaticStream(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting, long stream,
                                               long epilogue, int NumberOfPartitions, int min_period_filter, int release_CPU) {
        return schedule(taskOnAllCPU, t_stream, prologue, splitting, stream, epilogue, NumberOfPartitions, min_period_filter, release_CPU, null, false, 0, 0, null);
    }
     ...
     */
    public static boolean scheduleStaticStream(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting, long stream,
                                               long epilogue, int NumberOfPartitions, int min_period_filter, int release_CPU, DataAllocationPolicy shuffle) {
        return schedule(taskOnAllCPU, t_stream, prologue, splitting, stream, epilogue, NumberOfPartitions, min_period_filter, release_CPU, shuffle, false, 0, 0, null);
    }
     ...
     */
    public static boolean scheduleStreamingStream(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting,
                                                  long stream, long epilogue, int NumberOfPartitions, int min_period_filter, int release_CPU, int MIT, double Latency) {
        return scheduleStreamingStream(taskOnAllCPU, t_stream, prologue, splitting, stream, epilogue, NumberOfPartitions, min_period_filter, release_CPU, null, MIT, Latency);
    }
     ...
     */
    public static boolean scheduleStreamingStream(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting,
                                                  long stream, long epilogue, int NumberOfPartitions, int min_period_filter, int release_CPU, DataAllocationPolicy shuffle, int MIT, double Latency) {
        meetLatency isMeet = new meetLatency();
        boolean batchDeadlineMeet = schedule(taskOnAllCPU, t_stream, prologue, splitting, stream, epilogue, NumberOfPartitions, min_period_filter, release_CPU, shuffle, true, MIT,
                Latency, isMeet);
        //System.out.println(isMeet.meetLatency + "\t" + batchDeadlineMeet);
        return isMeet.meetLatency & batchDeadlineMeet;
    }

    public static boolean schedule(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting, long stream,
                                   long epilogue, int NumberOfPartitions, int min_period_filter, int release_CPU, DataAllocationPolicy shuffle, boolean isStreaming, int MIT,
        ...
        if (shuffle == null) {
            shuffle = new ResponseTimeDataAllocationPolicy(guaranteed_C, stream / NumberOfPartitions, NumberOfPartitions, prologue + splitting, release_CPU, servers, taskOnAllCPU, t_stream);
        ...
        double R = WCRTAanalysiStream(taskOnAllCPU, t_stream, prologue, splitting, stream, epilogue, NumberOfPartitions, release_CPU, servers, true, shuffle,
        ...
        if (printInfos)
            System.out.printf("The stream processing task's response time:\t%.2f\t%s%n", R, check);
        ...
        if (printInfos)
            System.out.println("-----------------------Mitegating Pessimism---------------------------");
        if (Schedulable)
            if (printInfos) System.out.println("Static Stream is Schedulable");
            else if (printInfos) System.out.println("Static Stream is Not Schedulable");
        return Schedulable;
    }

    private static double WCRTAanalysiStream(List<List<PeriodicTask>> taskOnAllCPU, PeriodicTask t_stream, long prologue, long splitting, long stream,
        ...
        printInfo.append(String.format("Splitting finishes at:\t%.2f", R_splitting)).append("\n");

        /* partition calculation */
        List<List<Long>> partitionAllocations = new ArrayList<>();
        ...
        double R_stream = 0;
        for (int i = 0; i < partitionAllocations.size(); i++) {
            int partitionCount = partitionAllocations.get(i).size();
            double workload = stream * partitionCount / NumberOfPatitions;
            ...
            int partitionSize = NumberOfPatitions / NumberOfPatitions;
            if (isStreaming) {
                for (int partitionIndex = 0; partitionIndex < partitionAllocations.get(i).size(); partitionIndex++) {
                    ...
                    for (int itemIndex = 0; itemIndex < partitionSize; itemIndex++) {
                        ...
                        workload = stream * ItemsCount / NumberOfPatitions;
                        if (i == release_CPU) workload = workload + prologue + splitting;
                        ...
                        double WaitingTime = MIT * (NumberOfPatitions - (itemId + 1));
                        latencyRecords.put(itemId, R_Item + WaitingTime);
                    }
                }
            }

            printInfo.append("In processor ").append(i)
                    .append(String.format(" parallel processing finishes at:\t%.2f", R))
                    .append("\t number of partitions allocated: ").append(partitionCount).append("\n");
            if (R_stream < R) R_stream = R;
        }
        printInfo.append(String.format("All of the parallel processing finishes at:\t%.2f", R_stream)).append("\n");
        ...
        printInfo.append(String.format("The resposne time of the epilogue of the task:\t%.2f", R_epi));
        ...
        if (printInfomation) {
            ...
            if (isStreaming) {
                ...
                for (long i = 0; i < NumberOfPatitions; i++) {
                    ...
                    if (printInfos)
                        System.out.printf("\tItem %d's\tLatency =\t%.2f\t%s%n", i, latency, meetLatency);
                }
            }
        }
        return R;
