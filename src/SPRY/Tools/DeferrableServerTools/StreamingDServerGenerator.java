...
public class StreamingDServerGenerator {
    ...
    public ArrayList<DServer> serverGeneration(PeriodicTask t, double prologue, double epilogue, List<List<PeriodicTask>> taskOnAllCPU, int release_CPU,
    ...
    public ArrayList<DServer> serverGeneration(PeriodicTask t, double prologue, double epilogue, List<List<PeriodicTask>> taskOnAllCPU,
        ...
        List<PeriodicTask> tasks = taskOnAllCPU.get(release_CPU);
		...
		for (Long aLong : divisors) {
			...
			for (int DSPriority : DSPriorities) {
...
//            optServers.forEach(System.out::println);
...
//        }
    ...
    public DServer searchForRestProcessor(List<PeriodicTask> tasks, PeriodicTask t) {
        return searchForRestProcessor(tasks, t, 1);
    }

    public DServer searchForRestProcessor(List<PeriodicTask> tasks, PeriodicTask t, double minPeriod) {
        return searchForRestProcessor(tasks, t, minPeriod, 0);
    }

    public DServer searchForRestProcessor(List<PeriodicTask> tasks, PeriodicTask t, double minPeriod, double DSReleaseJitter) {
...
//					 +"\t&"+String.format("%1.1f",current_MAX_C_Guaranteed)+"\\\\");
    ...
    private ArrayList<Integer> deadlineMonotonic(ArrayList<PeriodicTask> hardTasks, double Given_Deadline, double Given_Release_Jitter) {
        ...
        for (PeriodicTask hardTask : hardTasks) if (hardTask.priority < priority) priority = hardTask.priority;
        priority -= 1;
